...
class _ImmersiveLiveMapCardState extends State<ImmersiveLiveMapCard> {
          ...
          children: [
            ...
            FlutterMap(
              ...
              children: [
                ...
                // Dynamic wind field visualization based on forecast time
                PolygonLayer(
                  polygons: _generateTimeBasedWindFields(
                      widget.hurricanes, widget.selectedTimeIndex),
                ),
  ...
  List<Polygon> _generateTimeBasedWindFields(
    ...
    for (final hurricane in hurricanes) {
      ...
      // Adjust radius based on time (storm evolution)
      final baseRadius = hurricane.windSpeed / 10;
      final timeMultiplier = 1.0 + (timeIndex / 48) * 0.5; // Grows over time
      ...
      windFields.add(Polygon(
        ...
        color: AppTheme.getHurricaneCategoryColor(hurricane.category)
            .withOpacity(
                0.15 + (timeIndex / 48) * 0.1), // Intensifies over time
    ...
    for (final hurricane in hurricanes) {
      ...
      for (int ring = 1; ring <= 4; ring++) {
        ...
        for (int j = 0; j < numArrowsInRing; j++) {
          ...
          final spiralOffset =
              (ring - 1) * (math.pi / 6); // 30 degrees per ring
